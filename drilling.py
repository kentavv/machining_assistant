...
def print_machining_parameters(m, spindle_limited, spindle_rpm, max_spindle_rpm, requested_spindle_rpm, feed_per_revolution, sfm, material_sfm, plunge_feedrate, max_plunge_feedrate, drill_diam, limits, drill_angle, depth):
    ...
    print(f'<h3>G-code programming</h3>'
          f'<p>All code assumes that drill bit point at z=0 is top of hole and absolute distance mode (G90) is enabled.'
          f' These are meant for LinuxCNC, but may be suitable with other CNC controllers.</p>')
    if False:
      print(f'<p>'
            f'Imperial (G20) expression<br>'
            f'<pre>'
      ...
      print(f'<table class="styled-table">'
            ...
            f'<td>Style</td>'
            f'<td>Basic</td>'
            f'<td>Drilling cycle</td>'
            f'<td>Peck drilling cycle</td>'
            f'</tr>'
            f'</thead>'
            f'<tr><td>Imperial (G20) expression</td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g0 z.1\n'
            f'g1 z-[{depth.m_as("in"):.4f}+{approach_distance.m_as("in"):.4f}] f{plunge_feedrate.m_as("in / min"):.1f}\n'
            f'g0 z.1'
            f'</pre></td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g90 g98 g81 r.1 z-[{depth.m_as("in"):.4f}+{approach_distance.m_as("in"):.4f}] f{plunge_feedrate.m_as("in / min"):.1f}\n'
            f'</pre></td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g90 g98 g83 r.1 z-[{depth.m_as("in"):.4f}+{approach_distance.m_as("in"):.4f}] q{(peck_distance).m_as("in"):.4f}  f{plunge_feedrate.m_as("in / min"):.1f}\n'
            f'</pre></td>'
            f'</tr>'
            f''
            f'<tr><td>Imperial (G20) explicit</td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g0 z.1\n'
            f'g1 z-{(depth + approach_distance).m_as("in"):.4f} f{plunge_feedrate.m_as("in / min"):.1f}\n'
            f'g0 z.1'
            f'</pre></td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g90 g98 g81 r.1 z-{(depth + approach_distance).m_as("in"):.4f} f{plunge_feedrate.m_as("in / min"):.1f}\n'
            f'</pre></td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g90 g98 g83 r.1 z-{(depth + approach_distance).m_as("in"):.4f} q{(peck_distance).m_as("in"):.4f}  f{plunge_feedrate.m_as("in / min"):.1f}\n'
            f'</pre></td>'
            f'</tr>'
            f''
            f'<tr><td>Metric (G21) expression</td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g0 z2.54\n'
            f'g1 z-[{depth.m_as("mm"):.3f}+{approach_distance.m_as("mm"):.3f}] f{plunge_feedrate.m_as("mm / min"):.0f}\n'
            f'g0 z2.54'
            f'</pre></td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g90 g98 g81 r2.54 z-[{depth.m_as("mm"):.3f}+{approach_distance.m_as("mm"):.3f}] f{plunge_feedrate.m_as("mm / min"):.0f}\n'
            f'</pre></td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g90 g98 g83 r2.54 z-[{depth.m_as("mm"):.3f}+{approach_distance.m_as("mm"):.3f}] q{(peck_distance).m_as("mm"):.3f} f{plunge_feedrate.m_as("mm / min"):.0f}\n'
            f'</pre></td>'
            f'</tr>'
            f''
            f'<tr><td>Metric (G21) explicit</td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g0 z2.54\n'
            f'g1 z-{(depth + approach_distance).m_as("mm"):.3f} f{plunge_feedrate.m_as("mm / min"):.0f}\n'
            f'g0 z2.54'
            f'</pre></td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g90 g98 g81 r2.54 z-{(depth + approach_distance).m_as("mm"):.3f} f{plunge_feedrate.m_as("mm / min"):.0f}\n'
            f'</pre></td>'
            f'<td><pre>'
            f'm3 s{spindle_rpm.m_as("rpm"):.0f}\n'
            f'g90 g98 g83 r2.54 z-{(depth + approach_distance).m_as("mm"):.3f} q{(peck_distance).m_as("mm"):.3f} f{plunge_feedrate.m_as("mm / min"):.0f}\n'
            f'</pre></td>'
            f'</tr>'
            f'</tbody></table>'
           )
